...
if security find-identity -v -p codesigning | grep -q "\"$SIGN_IDENTITY\""; then
  ...
  codesign --force -s "$SIGN_IDENTITY" \
    -i io.github.chuckuz.meetrec.whisper-cli "$APP/Contents/MacOS/whisper-cli"
  codesign --force -s "$SIGN_IDENTITY" \
    -i io.github.chuckuz.meetrec.llama-server "$APP/Contents/MacOS/llama-server"
  codesign --force -s "$SIGN_IDENTITY" \
    -i io.github.chuckuz.meetrec "$APP"
else
  echo "⚠️  Сертификат «$SIGN_IDENTITY» не найден — подпись ad-hoc."
  echo "   Права на запись экрана будут слетать при каждой пересборке!"
  codesign --force -s - -i io.github.chuckuz.meetrec.whisper-cli \
    -r='designated => identifier "io.github.chuckuz.meetrec.whisper-cli"' \
    "$APP/Contents/MacOS/whisper-cli"
  codesign --force -s - -i io.github.chuckuz.meetrec.llama-server \
    -r='designated => identifier "io.github.chuckuz.meetrec.llama-server"' \
    "$APP/Contents/MacOS/llama-server"
  codesign --force -s - -i io.github.chuckuz.meetrec \
    -r='designated => identifier "io.github.chuckuz.meetrec"' \
    "$APP"
fi
