...
- **Выбор модели под ваше железо** (шестерёнка в шапке) — Qwen 2.5 **3B / 7B / 14B** с подсказкой, что подойдёт вашему Mac по объёму памяти: «Рекомендуется», «Доступно», «Впритык» или «Не хватит памяти». Модель можно скачать заранее (с прогрессом) или удалить, чтобы освободить диск.
