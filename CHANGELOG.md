...
### Изменено
- **Идентификатор приложения — `io.github.chuckuz.meetrec`.** Настройки и токены
  Google переносятся автоматически при первом запуске (macOS может один раз спросить
  пароль связки ключей). macOS считает пересобранное приложение новым: права
  «Микрофон» и «Запись экрана» нужно выдать заново.

## [1.21] — 2026-07-18

### Добавлено
- **Итоги длинных встреч теперь по ВСЕЙ записи (map-reduce).** Раньше для длинного
  транскрипта в итоги попадали только начало и конец — середина опускалась. Теперь
  длинный транскрипт режется на куски, каждый обобщается отдельно, затем выжимки
  сводятся в единые итоги. Короткие встречи, как и раньше, обобщаются одним быстрым запросом.

### Исправлено
- **ИИ-функции (чат, итоги, поиск) иногда падали с ошибкой «не удалось загрузить
  модель».** Локальный сервер модели брал случайный порт и падал, если тот оказывался
  занят другим приложением (например, портом IDE). Теперь берётся гарантированно
  свободный порт у ОС, а при коллизии выбирается другой — старт стал надёжным.

## [1.20] — 2026-07-17

### Исправлено
- **Транскрибация длинных записей (>1 часа) больше не зависает.** Раньше вывод
  `whisper-cli` не вычитывался из stdout, буфер пайпа переполнялся (примерно на 30%)
  и процесс вставал намертво. Теперь stdout дренится — длина записи не ограничена.
- **Итоги и чат по длинным встречам.** Контекст локальной LLM увеличен с 16К до 32К
  токенов (нативный для Qwen 2.5), поэтому длинные транскрипты помещаются и итоги
  генерируются без ошибки «локальная модель вернула ошибку». Заодно приведена в
  порядок работа с потоками вывода `llama-server`, чтобы сервер не завис на долгой сессии.

## [1.19] — 2026-07-08

### Изменено
- **Источник видео выбирается при старте записи**: при включённом «Видео экрана»
  нажатие «Запись» открывает диалог «Что записывать?» с живым списком окон (весь
  экран / монитор / окно) — окно удобнее выбирать в тот момент, когда оно открыто.
  Меню источника из карточки убрано. Если видео выключено — запись стартует сразу.
